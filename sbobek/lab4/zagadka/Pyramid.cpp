   ...
   for(int level = height; level >= 1; level--) {
      for(int index = 1; index <= level; index++) {
         if(done == 0 && done1 == 0 && get(level, index) == 0) {
            ...
            if(done == 0 && level > 1 && index < level && (help1 = get(level - 1, index)) > 0
                  && (help2 = get(level, index + 1)) > 0) {
               done1 = 1;
               fill(help1, help2, done, level, index);
            }
            if(done == 0 && level > 1 && index > 1 && (help1 = get(level - 1, index - 1)) > 0
                  && (help2 = get(level, index - 1)) > 0) {
               done1 = 1;
               fill(help1, help2, done, level, index);
            }
...
bool Pyramid::check_block(int level, int index) {
   ...
   if(level < height) {
      ...
      if(!(help3 == help1 + help2 || help1 == help2 + help3 || help2 == help1 + help3))
         ok = false;
   }
...
bool Pyramid::check_oth(int level, int index) {
   ...
   if(level < height) {
      help1 = get(level + 1, index);
      help2 = get(level + 1, index + 1);
      if(help1 != 0 && help2 != 0 && !(help3 == help1 + help2 || help1 == help2 + help3 || help2 == help1 + help3))
         ok = false;
   }
   if(level > 1 && index < level) {
      help1 = get(level - 1, index);
